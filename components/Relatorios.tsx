...
const Relatorios: React.FC<RelatoriosProps> = ({ alunos, turmas, presencas, matriculas = [], experimentais = [], user }) => {
  ...
  const [filtroStatusSec, setFiltroStatusSec] = useState<'todos' | 'ativos' | 'saidas' | 'leads'>('todos');
  ...
  const statsSecretaria = useMemo(() => {
    return alunos.filter(aluno => {
      ...
      const isLeadReal = aluno.statusMatricula === 'Lead' && !!aluno.fezExperimental;
      const isCancelado = aluno.statusMatricula === 'Cancelado';

      // Aplicar filtros de status
      if (filtroStatusSec === 'ativos' && !isAtivoNoContexto) return false;
      if (filtroStatusSec === 'saidas' && !isCancelado) return false;
      if (filtroStatusSec === 'leads' && !isLeadReal) return false;
      
      // Se filtrou por turma, só exibe quem tem matrícula naquelas turmas
      // Para Leads, permitimos se o plano bater
      const isLeadMatchTurma = isLeadReal && (filtroTurmaSec.length === 0 || filtroTurmaSec.some(ft => normalizeText(aluno.plano).includes(normalizeText(ft))));
      if (filtroTurmaSec.length > 0 && !isAtivoNoContexto && !isLeadMatchTurma) return false;
          ...
          <div className="bg-white p-10 rounded-[44px] shadow-sm border border-slate-100">
            <div className="flex items-center justify-between mb-8 pb-6 border-b border-slate-50">
               <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-sm font-black uppercase tracking-widest text-slate-800">Filtros de Pesquisa</h3>
               </div>
               <div className="flex items-center gap-2 px-4 py-2 bg-slate-50 rounded-xl border border-slate-100">
                  <Users className="w-3.5 h-3.5 text-blue-500" />
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Total Filtrado:</span>
                  <span className="text-sm font-black text-slate-900">{statsSecretaria.length}</span>
               </div>
            </div>
              ...
              <div className="w-full lg:w-96 space-y-3">
                ...
                <div className="flex bg-slate-100 p-1 rounded-3xl shadow-inner h-14">
                  {[
                    ...
                    { id: 'saidas', label: 'SAÍDAS', icon: UserX },
                      ...
                      <tr key={aluno.id} className="hover:bg-slate-50/50 transition-colors group">
                        ...
                        <td className="px-10 py-6">
                          <div className="flex flex-wrap gap-1.5 max-w-xs">
                            {cursos.length > 0 ? cursos.map((c, i) => (
                              <span key={i} className="px-2.5 py-1 bg-indigo-50 text-indigo-700 text-[9px] font-black uppercase rounded-lg border border-indigo-100 shadow-sm flex items-center gap-1">
                                <CheckCircle2 className="w-2.5 h-2.5" /> {c}
                              </span>
                            )) : aluno.statusMatricula === 'Lead' ? (
                              <span className="px-2.5 py-1 bg-blue-50 text-blue-700 text-[9px] font-black uppercase rounded-lg border border-blue-100 shadow-sm flex items-center gap-1">
                                <Zap className="w-2.5 h-2.5" /> LEAD: {aluno.plano || 'Interessado'}
                              </span>
                            ) : (
                              <span className="text-[10px] font-bold text-red-400 italic uppercase bg-red-50 px-2 py-1 rounded-md border border-red-100">Sem matrícula ativa</span>
                            )}
                          </div>
                        </td>
